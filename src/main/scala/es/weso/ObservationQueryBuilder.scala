...
object ObservationQueryBuilder extends App {
  
  def checkObservation(name:String, country:String, year:String) : (String, Double) =  {
    ...
    if(results.hasNext()){
      ...
      ( soln.get("obs").asResource().getURI(), soln.get("value").asLiteral().getDouble());
    } else throw new Exception("")
  }
  
  def checkIndicator(indicator:String, iType:String, country:String, year:String) : (String, Double) =  {
	val queryString : String = new StringBuilder()
	  .append("PREFIX wi-onto: <http://data.webfoundation.org/webindex/ontology/>\n")
	  .append("PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n")
	  .append("PREFIX qb: <http://purl.org/linked-data/cube#>\n")
